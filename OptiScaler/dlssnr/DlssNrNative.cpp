...
struct State{std::recursive_mutex mutex;bool enabled=false,restartRequired=false;std::string status="FP8 selected";uint64_t launches=0;std::map<NVDX_ObjectHandle,bool>modules;std::map<NVDX_ObjectHandle,Target>targets;std::map<ID3D12Device*,Device>devices;std::map<std::tuple<ID3D12GraphicsCommandList*,ID3D12Device*,uint64_t>,Session>sessions;
...
NvAPI_Status __cdecl Launch(ID3D12GraphicsCommandList*c,const NVAPI_CU_KERNEL_LAUNCH_PARAMS*k,NvU32 count){auto&s=S();std::lock_guard<std::recursive_mutex>g(s.mutex);if(s.restartRequired)return NVAPI_ERROR;if(!s.enabled)return s.launch(c,k,count);if(!k||count!=1){auto pending=std::find_if(s.sessions.begin(),s.sessions.end(),[&](const auto&v){return std::get<0>(v.first)==c&&v.second.pending;});if(pending!=s.sessions.end()){s.restartRequired=true;s.status="Restart required: unsupported launch chain while hybrid pair pending; precision change unavailable";fprintf(stderr,"%s\n",s.status.c_str());return NVAPI_ERROR;}return s.launch(c,k,count);}auto target=s.targets.find(k->hFunction);if(target==s.targets.end())return s.launch(c,k,count);auto&t=target->second;
 ...
 auto&d=s.devices[t.device];try{Prepare(d,t.device,c);AllocateSession(cycle,d,c);for(unsigned preflightBlock=0;preflightBlock<8;++preflightBlock){
...
void SetEnabled(bool on){auto&s=S();std::lock_guard<std::recursive_mutex>g(s.mutex);if(s.restartRequired){s.status="Restart required: hybrid recording failed; precision change was not applied";return;}if(s.enabled==on)return;for(auto&p:s.sessions)if(p.second.pending){s.restartRequired=true;s.status="Restart required: hybrid pair still pending; precision change was not applied";return;}s.enabled=on;s.status=on?"Hybrid FFN selected; waiting for supported M960/M2160/M3840 original model":"Original FP8 selected";}
